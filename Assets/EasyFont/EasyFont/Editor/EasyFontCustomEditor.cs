...
public class EasyFontCustomEditor : Editor {
	...
	public override void OnInspectorGUI()
	{
		...
		SerializedProperty serializedOrderInLayer 		= serializedObject.FindProperty("_privateProperties.orderInLayer");
		SerializedProperty serializedLineOfNum 			= serializedObject.FindProperty("_privateProperties.lineOfNum");
		...
		SerializedProperty serializedSpacing            = serializedObject.FindProperty("_privateProperties.spacing");
		...
		{	
			serializedText, serializedFontType, serializedFontFillMaterial , serializedFontSize, serializedCharacterSize, serializedOrderInLayer, serializedLineOfNum,serializedTextAnchor, serializedTextAlignment,
		...
		//OrderInLayer
		if(serializedOrderInLayer.isInstantiatedPrefab)
			SetBoldDefaultFont(serializedOrderInLayer.prefabOverride);

		customFont.OrderInLayer = EditorGUILayout.IntField(new GUIContent("Order In Layer", "Order In Layer"), customFont.OrderInLayer);

		//LineOfNum
		if(serializedLineOfNum.isInstantiatedPrefab)
			SetBoldDefaultFont(serializedLineOfNum.prefabOverride);
		
		customFont.LineOfNum = EditorGUILayout.IntField(new GUIContent("Line Of Num", "How many num of a line"), customFont.LineOfNum); 
		...
		//spacing
		if(serializedSpacing .isInstantiatedPrefab)
			SetBoldDefaultFont(serializedSpacing.prefabOverride);

		customFont.Spacing = EditorGUILayout.FloatField(new GUIContent("Spacing", "Distance between chars"), customFont.Spacing); 
		...
		EditorGUILayout.LabelField (string.Format("Font Texture Size {0} x {1}", customFont.GetComponent<Renderer>().sharedMaterial.mainTexture.width.ToString(),customFont.GetComponent<Renderer>().sharedMaterial.mainTexture.height.ToString()),greenText);
